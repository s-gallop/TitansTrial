...
bool check_collision_conditions(Entity entity_i, Entity entity_j) {
    if (registry.players.has(entity_i)) {
        ...
        if (registry.enemies.has(entity_j) || registry.ghouls.has(entity_j) || registry.spitterEnemies.has(entity_j) || registry.blocks.has(entity_j) || registry.spitterBullets.has(entity_j))
        {
            return true;
        }
